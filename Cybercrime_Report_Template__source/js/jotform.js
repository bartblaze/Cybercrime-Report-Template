...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    showAndResizeESignature: function(id) {
        // resize field and reset
        var element = $('id_' + id);
        if (element && element.select('.pad').length > 0) {
        ...
        var getValue = function (data, numeric) {
            ...
            switch (type) {
                ...
                case 'select':
                    ...
                    if ($('input_' + data).readAttribute('multiple') === 'multiple' || $('input_' + data).readAttribute('multiple') === '') {
                        if (numeric) val = 0;

                        $('input_' + data).select('option').each(function (option, ind) {
                            var option = $('input_' + data).options[ind];
                            if (option && option.selected) {
                                var current = option.readAttribute('data-calcvalue') ? option.readAttribute('data-calcvalue') : option.innerHTML.trim();
                                if(numeric) {
                        ...
                        if (option) {
                            val = option.readAttribute('data-calcvalue') ? option.readAttribute('data-calcvalue') : option.innerHTML.trim();
                        }
                    }
                    break;
